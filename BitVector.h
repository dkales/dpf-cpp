...
// In the underlying representation, bit 0 is the LSB of byte 0, bit 7 is the MSB of byte 0, bit 8 is the LSB of byte 1 etc
class BitVector
{
public:
    ...
    BitVector(uint8_t* data, uint64_t length, bool rev = false);
    ...
    // Append length bits pointed to by data starting a the bit index by offset, in reverse byte order
    ...
    // Returns a byte consisting of 8 bits starting at the specified index, only works if aligned on byte border
    ...
    bool isSet(const uint64_t idx) const;
...
//
//    template<>
//    inline bool channelBuffResize<BitVector>(BitVector& container, uint64_t size)
//    {
