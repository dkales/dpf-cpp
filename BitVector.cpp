...
BitVector::BitVector(uint8_t * data, uint64_t length, bool rev /* = false */)
    :
    mData(nullptr),
    mNumBits(0),
    mAllocBytes(0)
...
bool BitVector::isSet(const uint64_t idx) const
{
    if (idx >= mNumBits) throw std::runtime_error("rt error at " LOCATION);
    return (mData[(idx / 8)] & (1UL << (idx %8))) != 0;
}
...
std::string BitVector::hexREV() const
{
    ...
    for (unsigned int i = 0; i < sizeBytes(); i++)
    {
        s << std::setw(2) << std::setfill('0') << int(REVERSE_BYTE_ORDER[mData[i]]);
    }
